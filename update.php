...
?>
<?php
$separateQuery = "SELECT `employeeupdate`FROM `rolemasterinfo` WHERE `roleid` = '$id'";
$separateResult = mysqli_query($conn,$separateQuery);
while($separateRow = mysqli_fetch_assoc($separateResult)){
    if($separateRow['employeeupdate'] == 1){
...
if(isset($_POST['update'])){
  ...
  $sql = "UPDATE `employcreation` SET `empid` = '$empid',`empname` ='$empname', `mobileno` ='$mobileno' ,
                                      `email` = '$email' ,`role` = '$role',`gender` = '$gender',`password` ='$password',`updatedby` ='$separateid',`updatedon` = '$date'
                                      WHERE  `empid` = '$empid' ";
